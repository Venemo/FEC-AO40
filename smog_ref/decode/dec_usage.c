...
void test_decode_data() {
  ...
  ao40_decode_data(raw, data, error);
...
void test_decode_data_debug() {
  FILE *fp;
  uint8_t raw[AO40_RAW_SIZE];          // Raw data to be decoded
  uint8_t conv[AO40_CONV_SIZE];        // Deinterleaved raw block with SYNC removed
  uint8_t dec_data[AO40_RS_SIZE];      // Viterbi output: RS blocks interleaved and scrambled
  uint8_t rs[2][AO40_RS_BLOCK_SIZE];   // RS blocks without leading padding zeros
  uint8_t data[AO40_DATA_SIZE];        // Decoded data
  ...
  ao40_decode_data_debug(raw, data, error, conv, dec_data, rs);
